...
export function App() {
  ...
  const [railView, setRailView] = useState<"flow" | "rationale">("flow");
  ...
  return <div className="stage" style={{ transform: `translate(-50%, -50%) scale(${stageScale})` }}><nav className="rail"><header className="case-header"><img src="/sekai-mark.png" alt="Sekai mark" /><div><div className="rail-kicker">Creation redesign</div><h1>Sekai creation flow</h1></div></header><aside className="how-to"><span>How to explore</span><p><b>Choose a step</b> to jump to its interaction, or <b>tap the phone</b> to explore freely.</p></aside><div className="rail-tabs"><button className={railView === "flow" ? "active" : ""} onClick={() => setRailView("flow")}>Walkthrough</button><button className={railView === "rationale" ? "active" : ""} onClick={() => setRailView("rationale")}>Rationale</button></div>{railView === "flow" ? <><div className="rail-highlight"><small>Design highlight · {currentBeat.title}</small><b>{currentBeat.highlight}</b></div><div className="rail-flow">{beatMeta.map((item, i) => <button key={item.id} className={beat === item.id ? "active" : ""} onClick={() => jump(item.id)}><span>0{i + 1}</span><strong>{item.title}</strong><small>{item.problem}</small></button>)}</div></> : <div className="rationale"><div className="rationale-intro"><small>Design thesis</small><b>Creation should feel trustworthy before it feels magical.</b></div>{rationale.map(([problem, solution], i) => <article key={problem}><span>0{i + 1}</span><div><b>{problem}</b><p>{solution}</p></div></article>)}</div>}<footer className="profile-footer"><div><b>Xiaoyang Hu</b><span>Product designer · AI experiences</span></div><div className="profile-links"><a href="https://xiaoyanghu.com/" target="_blank" rel="noreferrer">Portfolio ↗</a><a href="https://www.linkedin.com/in/xiaoyang-hu-elena" target="_blank" rel="noreferrer">LinkedIn ↗</a><a href="mailto:mshuxy@gmail.com">Email ↗</a></div></footer></nav>
